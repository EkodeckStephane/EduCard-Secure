...
from app.schemas.students import StudentCreateRequest, StudentUpdateRequest
...
def archive_student(db: Session, student_id: int, principal: CurrentPrincipal) -> Student:
    assert_student_scope(db, principal, student_id)
    student = db.get(Student, student_id)
    if not student:
        raise HTTPException(status_code=404, detail="Student not found")
    ...
    student.record_version += 1
    db.add(StudentStatusHistory(student_id=student.id, previous_status=previous, new_status="ARCHIVED", reason="ARCHIVED", changed_by=principal.user.id))
    record_security_event(db, "STUDENT_ARCHIVED", "HIGH", principal.user.id, "student", student.public_id)
    db.commit()
    db.refresh(student)
    return student
...
def transfer_student(db: Session, student_id: int, to_school_id: int, to_classroom_id: int | None, comment: str | None, principal: CurrentPrincipal) -> Transfer:
    assert_student_scope(db, principal, student_id)
    ...
    student.record_version += 1
    db.add(transfer)
    record_security_event(db, "STUDENT_TRANSFERRED", "HIGH", principal.user.id, "student", student.public_id, comment)
    db.commit()
    db.refresh(transfer)
    return transfer
